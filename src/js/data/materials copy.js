...
const boxMaterials = palette.map((color, index) => {
    // Determine emissiveIntensity based on the index
    let emissiveIntensity = 0.1;
    if (index < 2) emissiveIntensity = 0.5;
...
const pinMaterials = Array.from({ length: 50 }, (_, index) => {
    if (index === 2 || (index >= 10 && index <= 49 && (index % 10) < 10)) {
        return generateMaterial(index);
    }
    return undefined;
});
export { pinMaterials };
console.log(pinMaterials)
