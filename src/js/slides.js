 export function updateSlide() {
    ...
    // Only the new slide's staged elements
    let newAppears = Array.from(allElements).filter(el => !Array.from(oldSlideElements).includes(el));
    if (newAppears.length > 0) {
        let currentPoint = 0;

        // Make all elements initially invisible
        newAppears.forEach(element => {
            element.style.opacity = "0";
        });

    // For slides with staged .appear elements, block "down" navigation
    // until all appear steps have been revealed.
    window.actionsCompleted = false

        window.slideEvents = {
            handleClick: function () {
                if (currentPoint < newAppears.length) {
                    newAppears[currentPoint].style.opacity = "1";
                    currentPoint++;
                    if (currentPoint == newAppears.length) {
        ...
        // No staged elements -> allow immediate navigation (e.g., image-only slides)
        window.actionsCompleted = true;
    }
